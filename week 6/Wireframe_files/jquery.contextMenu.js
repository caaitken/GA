  ...
  function createMenu(e) {
    ...
    settings.items.forEach(function(item) {
      if (item) {
        ...
        row.find('span').text(item.label).attr("shrtct", item.short);
        if (item.action) {
    ...
    // disabled manu items
    var s_el = $('.selected').length;
    if (s_el==1 && !$('.selected.grouped').length) {
      $('.contextMenuPlugin').find('span').removeClass('greyish');
      
      $('span[shrtct="Ctrl+G"]').addClass('greyish');

    }else if (s_el>1 || (s_el==1 && $('.selected.grouped').length)) {
      $('.contextMenuPlugin').find('span').removeClass('greyish');

    }else{
      $('.contextMenuPlugin a span').not($('span[shrtct="Ctrl+V"]')).addClass('greyish');
